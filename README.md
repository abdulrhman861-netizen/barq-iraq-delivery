...
## 📦 مرحلة الطلب والتوصيل (Firestore)

تم تفعيل تدفق الطلبات في لوحة الويب/الجوال ضمن تبويب **الطلبات والتوصيل**:

- إنشاء طلب حقيقي في مجموعة `orders`
- سجل الطلبات الخاص بالمستخدم الحالي (Realtime + زر تحديث يدوي)
...
- انتقالات الحالة المدعومة:
  ...
  - الإلغاء مسموح فقط قبل الإغلاق النهائي

نموذج الطلب المستخدم في Firestore قابل للتوسع للوحة الإدارة والتاجر في المرحلة القادمة، ويتضمن:

- `id`
- `createdBy`, `customerId`, `merchantId`
- `assignedCaptainId` (nullable)
...
5. افتح نفس تبويب **الطلبات والتوصيل**:
   - اقبل الطلب من قائمة الطلبات المتاحة.
   - حرّك الحالة بالتسلسل: `accepted` → `picked_up` → `in_transit` → `delivered`.
6. تأكد أن التغييرات تظهر مباشرة في سجل الطلبات.

### 🔐 ملاحظات قواعد Firestore الأمنية (مهم)
